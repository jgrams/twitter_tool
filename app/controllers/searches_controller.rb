class SearchesController < ApplicationController
  ...
  def show
    ...
    #if there is a username and it already has content, just look up the database version.
    if current_user.searches.find_by(username: params[:username]).username  && !current_user.searches.find_by(username: params[:username]).at_tweet_count.empty?
      search = current_user.searches.find_by(username: params[:username])
